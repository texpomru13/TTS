...
# ./montreal-forced-aligner/bin/mfa_train_and_align AnastasiyaV_ru/mfa_input AnastasiyaV_ru/dict_mfa.txt AnastasiyaV_ru/mfa_outputs -t ./montreal-forced-aligner/tmp -j 24
...
def transcribe_word_list(text):
    ...
    wordlist = text.split()
    wordilst_ac = accentizer_from_morpher_ru_wrapper(text).split()
    for word, word_ac in zip(wordlist,wordilst_ac):

        prediction = g2p.predict(word)
        # if word in letter and prediction:
